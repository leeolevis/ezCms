...
namespace DapperExtensions.Sql
{
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string GetPagingSql(string sql, int page, int resultsPerPage, IDictionary<string, object> parameters)
        {
        ...
        public virtual string SelectPaged(IClassMapper classMap, IPredicate predicate, IList<ISort> sort, int page, int resultsPerPage, IDictionary<string, object> parameters, string schemaName, string tableName, IList<IJoinPredicate> join, IList<IJoinAliasPredicate> alias)
        {
            ...
            var orderBy = sort.Select(s => s.GetSql(join != null, this, schemaName, tableName)).AppendStrings();
        ...
        public virtual string Insert(IClassMapper classMap, object entity, string schemaName, string tableName)
        {
            ...
            if (entity != null)
            {
                ...
                columns = columns.Where(p => fieldNames.Contains(p.Name));
            }
        ...
        public virtual string Update(IClassMapper classMap, object entity, IPredicate predicate, IDictionary<string, object> parameters, string schemaName, string tableName)
        {
            if (predicate == null)
            {
        ...
        public virtual string Delete(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters, string schemaName, string tableName)
        {
            ...
            var sql = new StringBuilder(string.Format("DELETE FROM {0}", GetTableName(classMap, schemaName, tableName)));
        ...
        public virtual string BuildSelectColumns(IClassMapper classMap, string schemaName, string tableName, IList<IJoinPredicate> join = null, IList<IJoinAliasPredicate> alias = null)
        {
            if (join != null && join.Count > 0)
            {
                var joinMap = new Dictionary<string, JoinMapper>();
                ...
                var ListColumn = new List<JoinMapper>();
                ...
                foreach (var itemMap in joinMap)
                {
                    ...
                    for (var i = vMapPro.Count - 1; i >= 0; i--)
                    {
