...
namespace ez.Core.Helpers
{
    ...
    public class GpsHelper
    {
        ...
        private double PI = 3.1415926535897932384626;//Math.PI;
        private double xPI = 3.14159265358979324 * 3000.0 / 180.0;//Math.PI * 3000.0 / 180.0;



        public GpsHelper(DFMGps _v)
        {
            _dfm = _v;
            DfmToWgs84();
            Wgs84ToGcj02();
        ...
        {
            int Degree = System.Convert.ToInt16(Math.Truncate(d));//度
            d = d - Degree;
            int M = System.Convert.ToInt16(Math.Truncate((d) * 60));//分
            int S = System.Convert.ToInt16(Math.Round((d * 60 - M) * 60));
            if (S == 60)
            {
                M = M + 1;
                S = 0;
