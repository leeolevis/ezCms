...
namespace ez.Core.Helpers
{
    ...
    public class HexHelper
    {
        ...
        public static ulong X2H(string value, int fromRadix)
        {
            ...
            {
                char val = value[i];
                if (!baseChar.Contains(val.ToString()))
                {
                    throw new ArgumentException(string.Format("参数中的字符\"{0}\"不是 {1} 进制数的有效字符。", val, fromRadix));
                }

                result += (ulong)baseChar.IndexOf(val) * (ulong)Math.Pow(baseChar.Length, value.Length - i - 1);
            }
