...
namespace ez.Core.Develops
{
    ...
    public static class CodeTimer
    {
        ...
        public static void InvokeAndWriteAll(Action action)
        {
            ...
            WriteLine(string.Format("\t总运行时间：{0}ms\t", stopwatch.Elapsed.TotalMilliseconds));
            WriteLine(string.Format("\tCPU运行时间:{0}ms\t", msecs));
            WriteLine(string.Format("\t测量实例得出的总运行时间:{0}ms\t", stopwatch.Elapsed.TotalMilliseconds));
        ...
        /// 执行Name输出
        /// <param name="action">执行方法</param>
        public static void CodeExecuteTime(string name, Action action)
        {
            if (string.IsNullOrEmpty(name))
            {
                return;
