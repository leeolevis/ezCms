...
using ez.Core.Contexts;
using ez.Core.Events.Handlers;
using ez.Core.Helpers;
using ez.Core.Reflections;

namespace ez.Core.Dependency {
    /// <summary>
    /// 依赖配置
    /// </summary>
    public class DependencyConfiguration {
        ...
        public IServiceProvider Config() {
            return IocHelper.DefaultContainer.Register( _services, RegistServices, _configs );
        }
        ...
        private void ResolveDependencyRegistrar() {
            ...
            types.Select( type => ReflectionHelper.CreateInstance<IDependencyRegistrar>( type ) ).ToList().ForEach( t => t.Regist( _services ) );
        }
